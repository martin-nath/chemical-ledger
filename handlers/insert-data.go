...
// 	dateRange := entry.Date > time.Now().Format("2006-01-02")

// 	// Validate required fields
// 	if entry.Type == "" || dateFormatErr != nil || dateRange != false || entry.CompoundName == "" ||
...
// 	err := db.Db.QueryRow("SELECT net_stock FROM compound WHERE id = ?", chemicalID).Scan(&currentStock)
...
// 			var count int
// 			row := db.Db.QueryRow("SELECT COUNT(*) FROM compound WHERE name = ?", entry.CompoundName).Scan(&count)
// 			if row == nil {
...
// 		case entryIncoming:
// 			insertQuery := `
//                 INSERT INTO compound (id, name, net_stock)
//                 VALUES (?, ?, ?)
//             `
...
// 	case err != nil:
// 		log.Println("Error fetching compound:", err)
...
// 			// if count  == 0 {
// 			// 	log.Println("Chemical not found:", chemicalID)
...
// 	insertentryQuery := `
//         INSERT INTO chemicals (id, type, date, chemical_id, no_of_units, quantity_per_unit, unit, remark, voucher_no)
//         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
//     `
